...
void ComputePmeMgr::submitReductions() {
    ...
    for ( int g=0; g<numGrids; ++g ) {
      ...
      if (alchFepOn) {
        ...
        if(simParams->alchFepWhamOn) {
          ...
          elecLambda3Down = simParams->getElecLambda(1.-simParams->alchLambda3());
            // elecLambda3Up =  (simParams->alchLambda3() <= alchElecLambdaStart)? 0. : \
            //          (simParams->alchLambda3() - alchElecLambdaStart) / (1. - alchElecLambdaStart);
            //    elecLambda3Down =  ((1-simParams->alchLambda3()) <= alchElecLambdaStart)? 0. : \
            //          ((1-simParams->alchLambda3()) - alchElecLambdaStart) / (1. - alchElecLambdaStart);
        }
      ...
      reduction->item(REDUCTION_ELECT_ENERGY_SLOW_F_REVERSE) += evir[g][0] * scale3;
      //reduction->item(REDUCTION_ELECT_ENERGY_SLOW_F_REVERSE) += 0;
