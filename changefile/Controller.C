...
void Controller::integrate(int scriptTask) {
  ...
  if ( scriptTask == SCRIPT_RUN ) {
    ...
    if (simParams->alchDoubleWide)
	{
	iout << "Using DoubleWide FEP\n";
    	printFepMessage_DoubleWide(step);
	}
    else
	{
	iout << "Not using DoubleWide FEP\n";
    ...
    if (simParams->alchDoubleWide)
	outputFepEnergy_DoubleWide(step);
    else
	...
	if(step % simParams->alchCalcFreq == 0)
	{
            if (simParams->alchDoubleWide)
	    	outputFepEnergy_DoubleWide(step);
	    else
...
void Controller::printFepMessage_DoubleWide(int step)
{
  if (simParams->alchOn && simParams->alchFepOn 
	...
	iout << "FEP: RESETTING FOR NEW FEP WINDOW "
         << "LAMBDA SET TO " << alchLambda << " LAMBDA2 " << alchLambda2 << "LAMBDA3" << alchLambda3
...
//fepb
      ...
      bondedEnergyDiff_f_reverse = reduction->item(REDUCTION_BONDED_ENERGY_F_REVERSE);
      ...
      if(simParameters->alchDoubleWide)
	{
	      electEnergy_f_reverse = reduction->item(REDUCTION_ELECT_ENERGY_F_REVERSE);
	      ljEnergy_r = reduction->item(REDUCTION_LJ_ENERGY_F_REVERSE);
	}
...
//fepe
    ...
    if ( minimize || ! ( step % slowFreq ) )
    {
      ...
      if(simParameters->alchDoubleWide)
	{
	       electEnergySlow_f_reverse = reduction->item(REDUCTION_ELECT_ENERGY_SLOW_F_REVERSE);
	}
...
//fepe
    ...
    if (simParameters->LJcorrection && volume) {
      ...
      //DoubleWide FEP
      if(simParameters->alchDoubleWide)
      	ljEnergy_r += molecule->getEnergyTailCorr(simParams->alchLambda3()) / volume;
...
//Modifications for alchemical fep
void Controller::outputFepEnergy_DoubleWide(int step)
{
 if (simParams->alchOn && simParams->alchFepOn) {
  ...
  if (alchEnsembleAvg && (stepInRun == 0 || stepInRun == alchEquilSteps)) {
    FepNo = 0;
    exp_dE_ByRT = 0.0;
    exp_dE_reverse_ByRT=0.0;
    net_dE = 0.0;
    net_dE_reverse=0.0;
  }
  dE = bondedEnergyDiff_f + electEnergy_f + electEnergySlow_f + ljEnergy_f -electEnergy - electEnergySlow - ljEnergy;

  BigReal dE_reverse =0;
  dE_reverse = bondedEnergyDiff_f + (electEnergy_f_reverse + electEnergySlow_f_reverse) - (electEnergy + electEnergySlow);
  BigReal RT = BOLTZMANN * simParams->alchTemp;

  if (alchEnsembleAvg){
    FepNo++;
    exp_dE_ByRT += exp(-dE/RT);
    exp_dE_reverse_ByRT += exp (-dE_reverse/RT);
    net_dE += dE;
    net_dE_reverse+=dE_reverse;
    //net_dE_reverse=net_dE_reverse+dE_reverse;
    //net_dE_reverse=0;
   }
  ...
  if (simParams->alchOutFreq) { 
    ...
    if ((simParams->N) && ((step%simParams->alchOutFreq) == 0)) {
      iout<<"Print DoubleWide FEP Energy\n";
      writeFepEnergyData_DoubleWide(step, fepFile);
      fepFile.flush();
    }
...
void Controller::writeFepEnergyData_DoubleWide(int step, ofstream_namd &file) {
  BigReal eeng = electEnergy + electEnergySlow;
  BigReal eeng_f = electEnergy_f + electEnergySlow_f;
  BigReal eeng_f_reverse = electEnergy_f_reverse + electEnergySlow_f_reverse;
  BigReal dE_Left = eeng_f + ljEnergy_f_left - eeng - ljEnergy;
  BigReal dE_reverse = (eeng_f_reverse + ljEnergy_r) - (ljEnergy+eeng);
  BigReal RT = BOLTZMANN * simParams->alchTemp;
  ...
  if(stepInRun){
    if(!FepWhamOn){
      iout<<"Print DoubleWide Energy 1";
      fepFile << FEPTITLE(step);
      fepFile << FORMAT(eeng);
      fepFile << FORMAT(eeng_f);
      fepFile << FORMAT(eeng_f_reverse);
      fepFile << FORMAT(ljEnergy);
      fepFile << FORMAT(ljEnergy_f);
    ...
    if( ! WCARepuOn ) {
      fepFile << FORMAT(dE);
      //fepFile << FORMAT(dE_reverse);
      fepFile << FORMAT(dE_reverse);
    }
    if(alchEnsembleAvg){
      BigReal dE_avg = net_dE/FepNo;
      //BigReal dE_reverse_avg=net_dE_reverse/FepNo;
      BigReal dE_reverse_avg=net_dE_reverse;
      fepFile << FORMAT(dE_avg);
      fepFile << FORMAT(dE_reverse_avg);
    }
    if(!FepWhamOn){
      fepFile << FORMAT(temperature);
    }
    if(alchEnsembleAvg){
      dG = -(RT * log(exp_dE_ByRT/FepNo));
      dG_reverse = -(RT * log(exp_dE_reverse_ByRT/FepNo));
      dG_reverse = 0;
      fepFile << FORMAT(dG);
      fepFile << FORMAT(dG_reverse);
    } 
    if( ! WCARepuOn ) {
      fepFile << std::endl;
    }
