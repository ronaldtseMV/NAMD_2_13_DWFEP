...
//fepb
  ...
  if ( ! strncasecmp(param,"alchCalcFreq",MAX_SCRIPT_PARAM_SIZE) ) {
    alchCalcFreq = atoi(value);
    ComputeNonbondedUtil::select();
    return;
  }

  SCRIPT_PARSE_BOOL("alchDoubleWide",alchDoubleWide);
...
//fepb
   ...
   opts.optional("alch", "alchCalcFreq",
     "Frequency of performing FEP calculation", &alchCalcFreq, 1);
   opts.range("alchLambdaFreq", NOT_NEGATIVE);
   ...
   opts.optionalB("alch","alchDoubleWide", "Enable Double Wide Sampling?",&alchDoubleWide,FALSE); 
...
BigReal SimParameters::alchLambda3()
{
 BigReal temp=alchLambda-(alchLambda2-alchLambda);

 if (temp < 0) temp = 0;

 if(temp==alchLambda && alchLambda!=0) temp = temp-0.02;
