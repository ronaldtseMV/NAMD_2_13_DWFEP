...
//DW FEP : Add reverse reduction term
 if(Node::Object()->simParameters->alchDoubleWide)
 {
  reduction->item(REDUCTION_ELECT_ENERGY_F_REVERSE) += data[electEnergyIndex_s_reverse];
  reduction->item(REDUCTION_ELECT_ENERGY_SLOW_F_REVERSE) += data[fullElectEnergyIndex_s_reverse];
  reduction->item(REDUCTION_LJ_ENERGY_F_REVERSE) += data[vdwEnergyIndex_s_reverse];   
 }
